from functui import NavState, NavAction, vnav, hnav

parse_event = lambda e: (e, None)

def test_nav_up_and_down():
    nav = NavState()
    tree=vnav("a", "b", "c")

    nav = nav.update(nav_tree=tree, event=NavAction.NAV_DOWN, parse_event_func=parse_event)
    ...
    nav = nav.update(nav_tree=tree, event=NavAction.NAV_UP, parse_event_func=parse_event)
    assert nav.is_active("b")

    # those should not do anything.
    nav = nav.update(nav_tree=tree, event=NavAction.NAV_LEFT, parse_event_func=parse_event)
    assert nav.is_active("b")

    nav = nav.update(nav_tree=tree, event=NavAction.NAV_RIGHT, parse_event_func=parse_event)
    assert nav.is_active("b")
